...
\begin{definition}[Orbit] \label{def:orbit} \uses{lem:darst_von_rot_res}
...
\end{lemma}
\begin{proof} \uses{theorem:auswahlaxiom}
...
\begin{lemma}[Abzählbarkeit von G] \label{lemma:G_abzählbar}
$G$ ist abzählbar.
\end{lemma}
\begin{proof} \uses{konk_rot_erzeugt}
Steht noch aus.
\end{proof}
...
\end{lemma}
\begin{proof} \uses{konk_rot_erzeuger konk_rot}
Steht noch aus.
\end{proof}
...
\end{lemma}
\begin{proof} \uses{def:menge_fixpunkte lemma:G_abzählbar lemma:eine_rot_achse}
Steht noch aus.
\end{proof}

\begin{lemma}[Lebesgue Maß D] \label{lemma:lebesgue_maß_D}
...
\begin{definition}[Vereinigung X] \label{def:vereinigung_x} \uses{def:menge_rep_punkte konk_rot}
$X=\bigcup\limits_{k=1}{\infty}A^{-k}M$. $X$ ist also die Menge aller Elemente von $M$, welche ausschließlich aus Rotationen
um $A^{-1}$ bestehen.
\end{definition}

\begin{definition}[Zerlegung in Mengen] \label{def:zerlegung_L'\D} \uses{def:menge_rep_punkte def:vereinigung_x konk_rot}
...
\end{lemma}
\begin{proof} \uses{def:zerlegung_L'\D def:menge_fixpunkte def:kugel_ohne_mittelpunkt}
Steht noch aus.
\end{proof}
...
\end{lemma}
\begin{proof} \uses{def:zerlegung_L'\D konk_rot}
Steht noch aus.
\end{proof}

\begin{lemma}[Verdopplung L'\D] \label{lemma:verdopplung_L'\D}
$L'\backslash D=P_1\cup AP_2$
$L'\backslash D=P_3\cup BP_4$
\end{lemma}
\begin{proof} \uses{lemma:rot_zerlegte_mengen}
Steht noch aus.
\end{proof}
...
\begin{definition}[Äquidekomponierbar] \label{def:äquidekomponierbar} 
...
\begin{lemma}[Äquidekomponierbarkeit von L'\D und L'] \label{lem:äquidekomponierbarkeit}
$L'\backslash D$ und $L'$ sind Äquidekomponierbar
\end{lemma}
\begin{proof}\uses{def:äquidekomponierbar lem:abz_fixpunkte}
...
\begin{lemma}[Äquidekomponierbarkeit Kreis] \label{lemma:äqui_kreis}
Ein Kreis ist äquidekomponierbar mit einem Kreis ohne einen bestimmten Punkt.
\end{lemma}
\begin{proof} \uses{def:äquidekomponierbar}
...
\begin{theorem}[Äquidekomponierbarkeit Kugel] \label{theorem:äqui_kugel}
Eine Kugel ohne ihren Mittelpunkt ist äquidekomponierbar mit der vollständigen Kugel.
\end{theorem}
\begin{proof} \uses{lemma:äqui_kreis}
...
\end{theorem}
\begin{proof} \uses{lemma:verdopplung_L'\D lem:äquidekomponierbarkeit lemma:äqui_kugel}
